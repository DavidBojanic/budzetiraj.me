...
$default_categories = ['Rent', 'Utilities', 'Groceries', 'Transportation', 'Entertainment', 'Healthcare', 'Savings', 'Other'];
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['delete_id'])) {
        ...
        if ($stmt->execute()) {
            echo "<div class='alert alert-success' id='success-message'>Expense deleted successfully!</div>";
        } else {
        ...
        $stmt = $conn->prepare("UPDATE recurring_expenses SET end_date = ? WHERE id = ? AND user_id = ?");
        $stmt->bind_param("sii", $current_date, $stop_id, $user_id);

        if ($stmt->execute()) {
            echo "<div class='alert alert-success' id='success-message'>Recurring expense stopped successfully!</div>";
        } else {
            echo "<div class='alert alert-danger'>Error: " . $stmt->error . "</div>";
        }
        ...
        if ($is_recurring) {
            $stmt = $conn->prepare("INSERT INTO recurring_expenses (user_id, category, amount, start_date, end_date, `interval`, description) VALUES (?, ?, ?, ?, ?, ?, ?)");
        ...
        if ($stmt->execute()) {
            echo "<div class='alert alert-success' id='success-message'>Expense added successfully!</div>";
        } else {
            echo "<div class='alert alert-danger'>Error: " . $stmt->error . "</div>";
        }
...
$expense_entries = $conn->query("SELECT id, amount, category, date, description, 'regular' AS type, NULL AS `interval`, NULL AS end_date FROM expenses WHERE user_id = $user_id
UNION
SELECT id, amount, category, start_date AS date, description, 'recurring' AS type, `interval`, end_date FROM recurring_expenses WHERE user_id = $user_id
ORDER BY date DESC");
...
<div class="container mt-4">
    ...
    <h3 class="mt-4">Expense Entries</h3>
    <table class="table table-striped">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <form action="expenses.php" method="post" style="display:inline;">
